...
$(document).ready(function() {
	...
	// Counter
	var endDate = "July 19, 2014 09:00:00";
        $('.countdown.styled').countdown({
          date: endDate,
          render: function(data) {
			  var years = this.leadingZeros(data.years, 2);
			  if (years != '00') {
            $(this.el).html("<div><span>" + this.leadingZeros(data.years, 2) + " </span><strong>年</strong></div><div><span>" + this.leadingZeros(data.days, 3) + " </span><strong>天</strong></div><div><span>" + this.leadingZeros(data.hours, 2) + "  </span><strong>小时</strong></div><div><span>" + this.leadingZeros(data.min, 2) + "</span><strong>分钟</strong></div><div><span>" + this.leadingZeros(data.sec, 2) + " </span><strong>秒</strong></div>");
			  } else {
			  $(this.el).html("<div><span>" + this.leadingZeros(data.days, 3) + " </span><strong>天</strong></div><div><span>" + this.leadingZeros(data.hours, 2) + "  </span><strong>小时</strong></div><div><span>" + this.leadingZeros(data.min, 2) + "</span><strong>分钟</strong></div><div><span>" + this.leadingZeros(data.sec, 2) + " </span><strong>秒</strong></div>");
			  }
          }
        });
